...
cv::Point VideoFaceDetector::getFrameAndDetect(cv::Mat &frame)
{
    *m_videoCapture >> frame;
    ...
    m_scale = (double) std::min(m_resizedWidth, use.cols) / use.cols;
    cv::Size resizedFrameSize = cv::Size(int(m_scale * use.cols), int(m_scale * use.rows));

    cv::Mat resizedFrame;
    if (resizedFrameSize.width > use.cols)
        cv::resize(use, resizedFrame, resizedFrameSize, cv::INTER_CUBIC);
    else
        cv::resize(use, resizedFrame, resizedFrameSize, cv::INTER_AREA);
